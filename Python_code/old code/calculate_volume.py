...
df = pd.DataFrame(columns=['bp_volume', "pressure"])

directory_path = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/COMSOL/EDP_25"
...
for file_name in os.listdir(directory_path):
    ...
    if file_name.endswith('.vtu'):
        print(file_name)
        pressure = file_name.split('_')[1].split('.')[0][1:]
        # Full file path
        file_path = os.path.join(directory_path, file_name)

        # Process the file and store the output in the list
        bp_volume,myo_volume = cal_bp_volume(file_path)
        df.loc[len(df)] = [bp_volume*1e6, int(pressure)]


df_sorted = df.sort_values(by='pressure')
print(df_sorted)

df_sorted.to_csv('/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/COMSOL/EDP_25/P_volumes.csv', index=False)