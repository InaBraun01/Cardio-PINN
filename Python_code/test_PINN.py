...
for i in range(num_iterations):
    ...
    with tf.Session() as sess:  #session holds values of intermediate results and variables
        ...
        a_new = np.multiply(amplitude_max,sess.run(a_pred, feed_dict={p_tf:[[0.1,0.1]]})) #predicte using NN for given pressure and active stress
        volume = Compute_Volume(a_new) #compute the volume of the new shape

        print(f" The predicted volume  at 0.0 pressure is {volume}")
        print(f" The groundtruth volume is {ED_volume}")
        ...
        print(f" The predicted volume at EDP is {volume}")
