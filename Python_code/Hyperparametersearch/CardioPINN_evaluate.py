...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/Maike_hyperparams/Run2"
...
with tf.Session() as sess:  #session holds values of intermediate results and variables
    ...
    for i in range(n_steps): #for the 180 timesteps simulated
        if t[i]>diastole_length and t[i]<=diastole_length + systole_length:   #during systole
            ...
            active_stress[i-shift+1] = max_act*(1.0-(2*(t[i]-offset_time_)/systole_length)**2) #behaves like a parabolar opened downward 
