...
parser.add_argument("--max_act", type=float, default=0.85e5, help="Value for max_act")
parser.add_argument("--Windkessel_R", type=float, default=50.0, help="Value for Windkessel_R")
parser.add_argument("--Windkessel_C", type=float, default=5.0e-6, help="Value for Windkessel_C")
...
vtk_file = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/LV_mean_human.vtk"
...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/Vary_act_85_200kPa_C_5e-7_5e-6_R_5_50_V2"
...
max_act          = args.max_act # maximum actuation stress value [Pa] (was 0.85e5 or 1e5)
#max_act = 126134.28606267879
stiff_scale      = 0.75   # scaling value of shear moduli of the material model [-] 

# Circulation parameters
Windkessel_R  = args.Windkessel_R  # systemic circulation resistance (was 50)
Windkessel_C  = args.Windkessel_C # systemic circulation compliance (was 5.0e-6)
#Windkessel_C  = 5.0e-6 # systemic circulation compliance (was 5.0e-6)
...
with tf.Session() as sess:  #session holds values of intermediate results and variables
    ...
    plt.tight_layout()
    plt.savefig(out_folder +f'/Loss_function_{round(max_act)}_C{round(Windkessel_C)}_R{round(Windkessel_R)}.png',dpi=400) # save the plot
    plt.close()
...
#     saver.restore(sess, out_folder+'/Trained_model.ckpt')
    ...
    for csel in range(1,2):  #I really don't understand why the run through this loop 7 times. I think this loop can be getten rid off
        ...
        for i in range(0,n_steps): #go through all steps in the simulation
            ...
            if not systolic_phase:
                ...
                if ejection: #when in systolic ejection
                    ...
                    pressure_LV[i] = Isovolumetric_PressureUpdate(volume[i-1],active_stress[i])  #update the pressure while keeping the volumne constant
        ...
        plt.savefig(out_folder + f'/pV_{round(max_act)}_C{round(Windkessel_C)}_R{round(Windkessel_R)}.png')
        plt.close()

        df = pd.DataFrame({'volume':volume, 'pressure_LV':pressure_LV})
        df.to_csv(out_folder + f'/P_volumes_{round(max_act)}_C{round(Windkessel_C)}_R{round(Windkessel_R)}.csv', index=False) 
