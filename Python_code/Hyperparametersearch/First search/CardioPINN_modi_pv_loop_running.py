...
#Input file 
data_file = "Test_data/"
vtk_file = "../Test_data/double_Maike_ED.vtk"
...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/Maike_hyperparams/Run3"
...
max_act          = 4390217.540615827 # maximum actuation stress value [Pa]
...
stress_normalization   = max_act # scaling value for actuation stresses [Pa]
...
#     saver.restore(sess, out_folder+'/Trained_model.ckpt')
    ...
    for csel in range(1,2):  #I really don't understand why the run through this loop 7 times. I think this loop can be getten rid off
        ...
        plt.plot(scaled_volume[:max_i],pressure_LV[:max_i]) #plot ans save LV loop
...
#             ccdumb+=1
        ...
        df = pd.DataFrame({'volume':scaled_volume[:max_i], 'pressure_LV':pressure_LV[:max_i]})
        df.to_csv(out_folder + '/P_volumes_hyper.csv', index=False) 
