...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/Vary_act_85_200kPa_V2"
...
Windkessel_R  = 50.0 # systemic circulation resistance (was 50)
...
with tf.Session() as sess:  #session holds values of intermediate results and variables
    ...
    plt.tight_layout()
    plt.savefig(out_folder +f'/Loss_function_{round(max_act)}.png',dpi=400) # save the plot
    plt.close()
...
#     saver.restore(sess, out_folder+'/Trained_model.ckpt')
    ...
    for csel in range(1,2):  #I really don't understand why the run through this loop 7 times. I think this loop can be getten rid off
        ...
        plt.savefig(out_folder + f'/pV_{round(max_act)}.png')
        plt.close()

        df = pd.DataFrame({'volume':volume, 'pressure_LV':pressure_LV})
        df.to_csv(out_folder + f'/P_volumes_{round(max_act)}.csv', index=False) 
