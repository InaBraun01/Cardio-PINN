...
def PressureUpdateSystole(active_s):
    ...
    while err > 0.001 and iterations < 10:
        ...
        residual_windkessel = -(volume[i-1]-V_lv_0)+ Windkessel_C *(p_0-pressure_LV[i-1])*133.32 + DT*p_0/Windkessel_R*133.32 #for the windkesselmodel this should be zero (I dont get the DT?)
        first_derivative    = LV_compliance + Windkessel_C  + DT/Windkessel_R #derivative of residual

        # NR iteration
        delta_p = - residual_windkessel/first_derivative/133.32 #iteratively change the pressure until the change in pressure is too small
...
#Input file 
data_file = "Test_data/"
vtk_file = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/double_Maike_ED/PINN_data/double_Maike_ED.vtk"
...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/Maike_hyperparams/Run8"
# file = open(out_folder +'Results_Parameters_run8.txt', 'w')
# # Step 2: Write something to the file

# file.write("Varied Windkessel C and Windkessel R\n")
...
end_systolic_LV_pressure = 100.0  -end_diastolic_LV_pressure  # end systolic left ventricular pressure value
diastolic_aortic_pressure = 24.  # end diastolic aortic pressure value
...
#     saver.restore(sess, out_folder+'/Trained_model.ckpt')
    ...
    for csel in range(num_iterations):  #I really don't understand why the run through this loop 7 times. I think this loop can be getten rid off
        Windkessel_C = np.random.uniform(2.50e-6,10e-6)
        #Windkessel_C = 4.493657336644564e-06
        print(Windkessel_C)
        print(type(Windkessel_C))
        Windkessel_R = np.random.uniform(25,75)
        #Windkessel_R = 52.98114271516078
        print(Windkessel_R)
        print(type(Windkessel_R))
        #max_act = np.random.uniform(0.85e5,5e6)

        print(f"Current parameters used: Windkessel_R {Windkessel_R}, Windkessel_C {Windkessel_C}")
        #print(f"Current parameters used: Max actove stress {max_act}")
        ...
        for i in range(n_steps): #for the 180 timesteps simulated
            if t[i]>diastole_length and t[i]<=diastole_length + systole_length:   #during systole
                shift = int(diastole_length/dt_) #shift so that simulation begins with isovolumetric contraction 
                active_stress[i-shift] = max_act*(1.0-(2*(t[i]-offset_time_)/systole_length)**2) #behaves like a parabolar opened downward 
        ...
        for i in range(0,n_steps): #go through all steps in the simulation
            ...
            if not systolic_phase:
                ...
                if ejection: #when in systolic ejection
                    ...
                    if syst_steps>3 and syst_steps<50: #after three steps in systole
        ...
        if not np.isnan(save_volume):
            if abs(save_volume - ESV) < distance:
                ...
                Best_R  = Windkessel_R
                Best_C  = Windkessel_C
                print(f"New Best params:  Best_R {Best_R}, Best_C {Best_C}")
               # print(f"New Best params: Windkessel_R {Best_R}, Windkessel_C {Best_C}, Best_max_act {Best_max_act}")
                ...
                print(f"Best params:  Best_R {Best_R}, Best_C {Best_C}")
                print(f"Distance: {distance}")

        df = pd.DataFrame({'volume':save_volume,'pressure_LV':pressure_LV})
        df.to_csv(out_folder + '/P_volumes_2.csv', index=False) 

    print(f"BEST PARAMETERS: Best_R {Best_R}, Best_C {Best_C}")
    print(f"OPTIMAL DISTANCE REACHED: Distance {distance}")
    ...
    print(f"DISTANCE RERUN: {abs(save_volume - ESV)}")
...
#     # print(f"Best params: Windkessel_R {Best_R}, Windkessel_C {Best_C}, Best_max_act {Best_max_act}")
...
#             print(f"New Best params: maximum active stress Best_R {Best_R}, Best_C {Best_C}")
#             # print(f"New Best params: Windkessel_R {Best_R}, Windkessel_C {Best_C}, Best_max_act {Best_max_act}")
#             print(f"ESV moved closer, the new distance: {distance}")
#             # if distance < 8*2: 
#                 # file.write(f"\n")
...
#                 # file.write(f"\n") 
#         else:
#             print(f"Best params: maximum active stress Best_R {Best_R}, Best_C {Best_C}")
#             print(f"Distance: {distance}")
...
# # file.write(f"Best params:  Best_R {Best_R}, Best_C {Best_C}\n")
# # file.write(f"Best distance: {distance} ml\n")
# # file.write(f"Best distance downscaled: {distance/8} ml")
