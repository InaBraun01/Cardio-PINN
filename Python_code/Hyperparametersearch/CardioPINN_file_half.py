...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/Diastolic_filling_Ta_60250"
...
max_act          = 60250.0 # maximum actuation stress value [Pa]
stiff_scale      = 0.75   # scaling value of shear moduli of the material model [-] 

# Circulation parameters
Windkessel_R  = 50.0/2  # systemic circulation resistance
Windkessel_C  = 5.0e-6/2 # systemic circulation compliance
...
num_a_iso = float(sys.argv[1])
num_b_iso = float(sys.argv[2])
num_a_f   = float(sys.argv[3])
num_b_f   = float(sys.argv[4])
num_a_s   = float(sys.argv[5])
num_b_s   = float(sys.argv[6])
num_a_fs  = float(sys.argv[7])
num_b_fs  = float(sys.argv[8])
num_Bulk  = 10.5e5
out_folder = f"/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/Diastolic_filling_Ta_60250_CR_half_a_iso{round(num_a_iso)}"
...
#     saver.restore(sess, out_folder+'/Trained_model.ckpt')
    ...
    for csel in range(1,2):  #I really don't understand why the run through this loop 7 times. I think this loop can be getten rid off
        ...
        plt.savefig(out_folder + f'/pV_loop.png')
        plt.close()
