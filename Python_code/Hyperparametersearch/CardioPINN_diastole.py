...
parser.add_argument("--factor_aiso", type=float, default=1.0, help="Factor with which a_iso is multiplied")
parser.add_argument("--factor_as", type=float, default=1.0, help="Factor with which a_s is multiplied")
parser.add_argument("--factor_af", type=float, default=1.0, help="Factor with which a_f is multiplied")
parser.add_argument("--factor_afs", type=float, default=1.0, help="Factor with which a_fs is multiplied")
parser.add_argument("--factor_biso", type=float, default=1.0, help="Factor with which b_iso is multiplied")
parser.add_argument("--factor_bs", type=float, default=1.0, help="Factor with which b_s is multiplied")
parser.add_argument("--factor_bf", type=float, default=1.0, help="Factor with which b_f is multiplied")
parser.add_argument("--factor_bfs", type=float, default=1.0, help="Factor with which b_fs is multiplied")
...
factor_aiso = args.factor_aiso
factor_af = args.factor_af
factor_as = args.factor_as
factor_afs = args.factor_afs
factor_biso = args.factor_biso
factor_bf = args.factor_bf
...
num_b_f   = 14.472 * factor_bf
num_a_s   = 0.481e3 * factor_as 
num_b_s   = 12.548 * factor_bs
num_a_fs  = 0.283e3 * factor_afs
...
# num_a_fs  = 0
# num_b_fs  = 1.0
# num_Bulk  = 10.5e5 #10.5e5

a_iso = tf.constant(num_a_iso,dtype=np.float32)
b_iso = tf.constant(num_b_iso,dtype=np.float32)
a_f   = tf.constant(num_a_f,dtype=np.float32)
b_f   = tf.constant(num_b_f,dtype=np.float32)
a_s   = tf.constant(num_a_s,dtype=np.float32)
b_s   = tf.constant(num_b_s,dtype=np.float32)
a_fs  = tf.constant(num_a_fs,dtype=np.float32)
b_fs  = tf.constant(num_b_fs,dtype=np.float32)
...
n_steps = int((diastole_length)/dt_) #number of time steps in simulation (only simulate diastolic filling)
t       = np.linspace(0,diastole_length,n_steps) #list of time steps for which simulation is done
