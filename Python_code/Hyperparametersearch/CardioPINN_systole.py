...
parser.add_argument("--max_act", type=float, default=0.85e5, help="Maximal activation stress")
parser.add_argument("--Windkessel_C", type=float, default=5.0e-6, help="Parameter used as compliance in the 2 element Windkessel model")
parser.add_argument("--Windkessel_R", type=float, default=50.0, help="Parameter used as resistance in the 2 element Windkessel model")
...
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/Diastolic_filling_022"
out_folder = "/data.lfpn/ibraun/Code/Cardio-PINN/Synthetic_shapes/LV_mean_human/Diastolic_filling_022"
...
#max_act          = 0.5e5 # maximum actuation stress value [Pa]
...
#Windkessel_R  = args.Windkessel_R 
Windkessel_C  = 5.0e-6 # systemic circulation compliance
#Windkessel_C  = args.Windkessel_C
    ...
    for csel in range(1,2):  #I really don't understand why the run through this loop 7 times. I think this loop can be getten rid off
        ...
        for i in range(0,n_steps): #go through all steps in the simulation
            ...
            if pressure_LV[i-1]>diastolic_aortic_pressure: #initiate systolic phase
                systolic_phase = True
                values = [max_act, active_stress[i-1], volume[i-1], max_volume - volume[i-1]]
                if j== 0:
                    with open('results_iso_contract.csv', 'a', newline='') as file:
        ...
        df.to_csv(out_folder + f'/P_volumes_{csel}.csv', index=False) 
